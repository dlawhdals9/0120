...
//		배열은 한 번 선언 하면 프로그램에서 크기를 변경할 수 없다. 수정수정수정수정수정 !!!!@@###@@@
		int [] data = new int[10];
		
//		ArrayList는 데이터가 입력되면 자동으로 크기가 커지고 데이터가 제거되면 자동으로 크기가 작아진다.
//		중간에 데이터가 삽입되면 데이터가 삽입되는 위치부터 모든 데이터가 뒤로 이동되고 중간의 데이터가 제거되면 제거된 다음 위치부터 
//		모든 데이터가 앞으로 이동된다.
		
		
//		<E> => 제네릭이라 부르면 ArrayList에 저장될 데이터의 타입을 반드시 클래스로 적는다. => 기본 자료형은 사용할 수 없다.
